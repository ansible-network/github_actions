...
def test_argparse():
    args = parse_args("--collection-path a,b --test-all-the-targets".split(" "))
    assert args.collection_path == "a,b"
    assert args.test_all_the_targets is True
...
def test_what_changed_git_call(m_subproc_popen):
    proc = MagicMock()
    m_subproc_popen.return_value = proc
    proc.communicate.return_value = b"plugins/modules/foo.py\n", b""
    whc = WhatHaveChanged(PosixPath("a"), "stable-2.1")
    whc.collection_name = lambda: "a.b"
    ["plugins/modules/foo.py"] == whc.changed_files()
    m_subproc_popen.assert_called_with(
        "git diff origin/stable-2.1 --name-only", stdout=-1, stderr=-1, shell=True, cwd=PosixPath("a")
    )
